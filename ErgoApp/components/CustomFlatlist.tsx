...
export interface CustomFlatlistItem {
  key: string | number;
  [key: string]: any;
}
...
interface CustomFlatlistProps {
  ...
  indicatorPostponedColor?: string;
...
const CustomFlatlist = forwardRef<CustomFlatlistRef, CustomFlatlistProps>(
  (
    ...
    useImperativeHandle(ref, () => ({
      scrollToIndex: (index: number, animated: boolean = true) => {
        if (flatListRef.current && index >= 0 && index < data.length) {
          // Use scrollToOffset which is more reliable
          const offset = index * ITEM_WIDTH;
          flatListRef.current.scrollToOffset({ offset, animated });

          // Ensure activeIndex is updated immediately for programmatic scrolling
          setActiveIndex(index);

          // Update the scrollX value to keep animations in sync
          if (animated) {
    ...
    const indicators = data.map((item, index: number) => {
      // Check if this item is postponed
      const isPostponed = (item as any).isPostponed;

      let backgroundColor;
      if (index === activeIndex) {
        backgroundColor = indicatorActiveColor;
      } else if (isPostponed) {
        backgroundColor = indicatorPostponedColor;
      } else {
      ...
      <View style={styles.wrapper}>
        <View style={[styles.container, containerStyle]}>
          <Animated.FlatList<CustomFlatlistItem>
            ...
            scrollEnabled={allowNavigation && !doExercises}
          />
        </View>
